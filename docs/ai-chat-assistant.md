...
-> backend returns a ready-made answer or AI formats the answer
```
...
- `AI_CHAT_USER_HOURLY_LIMIT`: max AI chat messages per user per hour. Default: `20`.
- `AI_CHAT_STORE_DAILY_LIMIT`: max AI chat messages per store per day. Default: `100`.
...
- 20 valid messages per user per hour.
- 100 valid messages per store per day.
...
## Ready-Made Responders / Token-Saving Mode

Simple supported questions are answered locally by backend responders after safe DB
tools have run. This avoids spending OpenAI tokens when the backend already has
enough structured business data to produce a clear answer.

## Basic Ready-Made Responders

Very short service messages are answered locally before intent routing, DB tools,
or OpenAI calls. This covers greetings, thanks, confirmations, and goodbyes in
English and Russian.

Examples:

- English greetings: `hi`, `hello`, `hey`, `good morning`, `good afternoon`,
  `good evening`.
- Russian greetings: `привет`, `здравствуйте`, `добрый день`, `доброе утро`,
  `добрый вечер`.
- English thanks: `thanks`, `thank you`, `thx`, `appreciate it`.
- Russian thanks: `спасибо`, `благодарю`, `спс`.
- English confirmations: `ok`, `okay`, `got it`, `understood`, `clear`.
- Russian confirmations: `ок`, `хорошо`, `понятно`, `ясно`, `понял`, `поняла`.
- English goodbyes: `bye`, `goodbye`, `see you`, `see you later`.
- Russian goodbyes: `пока`, `до свидания`, `увидимся`.

Basic matching trims whitespace, ignores simple punctuation, and uses exact
normalized phrases for short messages. If the message also contains business words
such as sales, stock, products, reports, category, restock, warehouse, analytics,
or their Russian equivalents, the assistant uses the normal business intent flow
instead. If the message contains a blocked technical request, such as database
schema or SQL, the guard refusal runs before the basic responder.

Basic ready-made responses keep `used_tools: []`, do not query DB tools, do not
call OpenAI, and do not add debug fields to the frontend response.

Local ready-made answers are used for:
...
The responder keeps the same public `POST /api/ai/chat` response shape and returns
only the final `answer`, `conversation_id`, and `used_tools`. It does not expose raw
tool results, SQL, schema, prompts, provider details, or debug data.

Language is chosen from the controller-provided language when available. If it is
missing, Cyrillic messages receive Russian answers and other messages receive
English answers. Money, numbers, percentages, and dates are formatted with
language-aware helpers such as `66,942.94 KZT` for English and `66 942,94 KZT`
for Russian.

OpenAI is still available for complex analytical questions, such as why sales went
down, comparisons, or unclear business questions that need a more natural
explanation across multiple tool results. If OpenAI fails, the existing safe
fallback answer path remains in place.

Token-saving mode preserves the same tenant protections: `store_id` still comes
only from the authenticated backend user, safe tools remain read-only and scoped to
the current store, and sanitizer validation still protects any context that may be
sent to OpenAI.
