...
export const RU_ERROR_MESSAGES = Object.freeze({
    ...
    [ERROR_CODES.DB_SCHEMA_COLUMN_MISSING]:
        ({ column = "unknown", table = "unknown" } = {}) =>
            `Ошибка схемы базы данных: колонка '${column}' отсутствует в таблице '${table}'.`,
    [ERROR_CODES.DB_SCHEMA_TABLE_MISSING]:
        ({ table = "unknown" } = {}) =>
    ...
    [ERROR_CODES.DB_UNIQUE_CONSTRAINT_VIOLATION]:
        "Нарушение уникальности данных.",

    [ERROR_CODES.AUTH_REQUIRED]: "Требуется аутентификация",
    [ERROR_CODES.AUTH_LOGIN_REQUIRED_FIELDS]:
    ...
    [ERROR_CODES.AUTH_PASSWORD_CONFIRM_MISMATCH]:
    ...
    [ERROR_CODES.AUTH_USER_EMAIL_EXISTS]:
        "Пользователь с таким email уже существует",
    [ERROR_CODES.AUTH_USER_PHONE_EXISTS]:
        "Пользователь с таким телефоном уже существует",
    [ERROR_CODES.AUTH_USER_NOT_FOUND]: "Пользователь не найден",
    [ERROR_CODES.AUTH_TOKEN_FORMAT_INVALID]:
    ...
    [ERROR_CODES.USERS_REQUIRED_FIELDS]:
    ...
    [ERROR_CODES.MOVEMENT_REQUIRED_FIELDS_OUT]:
    ...
    [ERROR_CODES.MOVEMENT_TYPE_INVALID]:
        ({ type } = {}) => `Недопустимый тип движения: ${type ?? "unknown"}`,
    [ERROR_CODES.MOVEMENT_PRODUCT_ID_INVALID]:
        "product_id обязателен и должен быть положительным числом",
    [ERROR_CODES.MOVEMENT_QTY_INVALID]:
        "qty обязателен и должен быть положительным числом",
    [ERROR_CODES.MOVEMENT_USER_ID_INVALID]:
        "user_id обязателен и должен быть положительным числом",
    ...
    [ERROR_CODES.MOVEMENT_WAREHOUSE_FROM_NOT_FOUND]:
        ({ warehouseId } = {}) =>
            `Склад-источник с ID ${warehouseId ?? "unknown"} не найден.`,
    [ERROR_CODES.MOVEMENT_WAREHOUSE_TO_NOT_FOUND]:
        ({ warehouseId } = {}) =>
            `Склад-назначение с ID ${warehouseId ?? "unknown"} не найден.`,
    [ERROR_CODES.MOVEMENT_STOCK_NOT_FOUND]:
        "Остаток не найден для данного товара и склада",
    [ERROR_CODES.MOVEMENT_STOCK_FROM_NOT_FOUND]:
        "Остаток не найден для склада-источника",
    [ERROR_CODES.MOVEMENT_STOCK_ADJUST_NOT_FOUND]:
        "Запись stock не найдена для корректировки",
    ...
    [ERROR_CODES.PRODUCT_SKU_REQUIRED]:
        "SKU товара обязателен и не может быть пустым",
    [ERROR_CODES.PRODUCT_PURCHASE_PRICE_INVALID]:
        "Цена закупки должна быть неотрицательным числом",
    [ERROR_CODES.PRODUCT_SALE_PRICE_INVALID]:
        "Цена продажи должна быть неотрицательным числом",
    ...
    [ERROR_CODES.PRODUCT_SKU_EXISTS]:
        ({ sku } = {}) => `Товар с SKU "${sku ?? ""}" уже существует`,
    [ERROR_CODES.PRODUCT_BARCODE_EXISTS]:
        ({ barcode } = {}) => `Товар с штрихкодом "${barcode ?? ""}" уже существует`,
    [ERROR_CODES.PRODUCT_NOT_FOUND]: "Товар не найден",
    [ERROR_CODES.PRODUCT_UNIQUE_CONSTRAINT]:
        "Нарушение уникальности данных.",
    [ERROR_CODES.PRODUCT_IMPORT_ARRAY_REQUIRED]:
        "Массив products обязателен",
    [ERROR_CODES.PRODUCT_IMPORT_ARRAY_EMPTY]:
        "Массив products не может быть пустым",
    [ERROR_CODES.PRODUCT_IMPORT_MAX_LIMIT]:
        "Максимум 1000 товаров за один импорт",
    [ERROR_CODES.PRODUCT_IMPORT_ROW_MISSING_FIELDS]:
        ({ row = "unknown", fields = [] } = {}) =>
            `Строка ${row}: отсутствуют обязательные поля: ${Array.isArray(fields) ? fields.join(", ") : fields}`,
    [ERROR_CODES.PRODUCT_IMPORT_ROW_SALE_PRICE_INVALID]:
        ({ row = "unknown" } = {}) =>
            `Строка ${row}: sale_price должен быть неотрицательным числом`,
    [ERROR_CODES.PRODUCT_IMPORT_ROW_SKU_EXISTS]:
        ({ row = "unknown", sku = "" } = {}) =>
            `Строка ${row}: SKU "${sku}" уже существует`,
    [ERROR_CODES.PRODUCT_IMPORT_ROW_BARCODE_EXISTS]:
        ({ row = "unknown", barcode = "" } = {}) =>
            `Строка ${row}: штрихкод "${barcode}" уже существует`,
    [ERROR_CODES.PRODUCT_IMPORT_ROW_PROCESS_FAILED]:
        ({ row = "unknown" } = {}) =>
            `Строка ${row}: ошибка при обработке товара`,
    [ERROR_CODES.PLATFORM_STORE_NAME_REQUIRED]:
        "Название магазина обязательно",
    [ERROR_CODES.PLATFORM_STORE_NOT_FOUND]:
        "Магазин не найден",
    [ERROR_CODES.PLATFORM_STORE_ID_INVALID]:
        "Некорректный идентификатор магазина",
    [ERROR_CODES.PLATFORM_STORE_STATUS_INVALID]:
        ({ status = "unknown" } = {}) =>
    ...
    [ERROR_CODES.PLATFORM_STORE_SLUG_EXISTS]:
        ({ slug = "" } = {}) =>
            `Магазин со slug "${slug}" уже существует`,

    [ERROR_CODES.NOTIFICATION_TYPE_REQUIRED]:
    ...
    [ERROR_CODES.NOTIFICATION_PAYLOAD_OBJECT_REQUIRED]:
        "Payload должен быть объектом",
    [ERROR_CODES.NOTIFICATION_NOT_FOUND_OR_ALREADY_READ]:
        "Уведомление не найдено или уже прочитано",

    [ERROR_CODES.SALES_ITEMS_REQUIRED]:
        "Список позиций продажи не может быть пустым",
    [ERROR_CODES.SALES_ITEM_PRODUCT_ID_REQUIRED]:
        "product_id обязателен для каждой позиции",
    [ERROR_CODES.SALES_ITEM_QTY_INVALID]:
        "qty должен быть положительным числом",
    [ERROR_CODES.SALES_ITEM_PRICE_INVALID]:
        "price обязателен и должен быть неотрицательным",
    [ERROR_CODES.SALES_WAREHOUSE_OR_STORE_REQUIRED]:
        "warehouse_id или store_id обязателен",
    [ERROR_CODES.SALES_INSUFFICIENT_STOCK]:
        ({ productId = "unknown", available = 0, required = 0 } = {}) =>
            `Недостаточно товара (product_id=${productId}) на складе. Доступно: ${available}, требуется: ${required}`,
    [ERROR_CODES.SALES_NOT_FOUND]:
        "Продажа не найдена",
    [ERROR_CODES.SALES_ALREADY_RETURNED]:
        "Продажа уже возвращена",
    [ERROR_CODES.SALES_NO_ITEMS]:
        "Продажа не содержит позиций",
    [ERROR_CODES.SALES_RETURN_WAREHOUSE_REQUIRED]:
        "warehouse_id обязателен для возврата",

    [ERROR_CODES.REPORTS_DATE_RANGE_REQUIRED]:
        "Параметры from и to обязательны (формат YYYY-MM-DD)",
    [ERROR_CODES.REPORTS_DATE_FORMAT_INVALID]:
        "Некорректный формат даты: используйте YYYY-MM-DD",
    [ERROR_CODES.REPORTS_DATE_RANGE_INVALID]:
        "Дата from должна быть меньше или равна дате to",

    [ERROR_CODES.API_ENDPOINT_NOT_FOUND]:
