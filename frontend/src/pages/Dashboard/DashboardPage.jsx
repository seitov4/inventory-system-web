...
import { getWidgetsByZone } from "./dashboardWidgets";
import {
    getDefaultLayout,
    loadLayout,
    saveLayout,
...
const ZoneOverlay = styled.div`
    ...
    pointer-events: none;
    z-index: 1;
    background: ${(props) => (props.$active ? "rgba(80, 140, 255, 0.08)" : "transparent")};
    border: ${(props) =>
        props.$active ? "1px dashed rgba(80, 140, 255, 0.25)" : "1px dashed transparent"};
    border-radius: 8px;
...
export default function DashboardPage() {
    ...
    }, [role, canSeeAnalytics, loading, layout.length]);
    ...
    }, [loading, allWidgets, role, canSeeAnalytics, layout]);
                ...
                <GridContainer>
                    ...
                    {isDragging &&
                        ...
                        (() => {
                            ...
                            return Object.entries(ZONE_BOUNDARIES).map(([zoneKey, boundaries]) => {
                                ...
                                const height =
                                    (boundaries.maxY - boundaries.minY) * (rowHeight + margin) -
                                    margin;
                    ...
                    >
                        {allWidgets.map((widget) => {
                            ...
                            if (!layoutItem) {
                                console.warn(
                                    `Widget ${widget.id} missing layout item. Total widgets: ${allWidgets.length}, Total layout items: ${layout.length}`
                                );
                                return null;
                            }
