...
class SalesPredictor:
    ...
    def forecast(self, raw_df: pd.DataFrame, model_name: str = "ensemble", horizon: int = 30) -> ForecastResult:
        ...
        for row, raw_prediction in zip(future.to_dict("records"), raw_predictions):
            ...
            prediction = self._limit_downtrend_rebound(history, store_id, prediction, step)
    ...
    def _blend_predictions(self, calibrated_prediction: float, seasonal_prediction: float) -> float:
        calibrated_valid = np.isfinite(calibrated_prediction) and calibrated_prediction >= 0
        seasonal_valid = np.isfinite(seasonal_prediction) and seasonal_prediction >= 0

        if not calibrated_valid and not seasonal_valid:
            return 0
        if not calibrated_valid:
            return max(0, seasonal_prediction)
        if not seasonal_valid:
    ...
    def _limit_downtrend_rebound(
        ...
        if len(store_history) < 10 or not np.isfinite(prediction):
            return max(0, prediction)

        recent = store_history.tail(min(21, len(store_history)))
        sales = recent["sales"].astype(float).to_numpy()
        last_actual = float(sales[-1])
        if last_actual <= 0:
            return max(0, prediction)

        x = np.arange(len(sales), dtype=float)
        slope = float(np.polyfit(x, sales, 1)[0])
        first_half = float(np.mean(sales[: max(1, len(sales) // 2)]))
        second_half = float(np.mean(sales[max(1, len(sales) // 2):]))
        drawdown = (first_half - second_half) / max(first_half, 1)
        is_downtrend = slope < 0 and drawdown > 0.12 and last_actual < float(np.median(sales))
        if not is_downtrend:
            return max(0, prediction)

        daily_noise = max(float(np.std(np.diff(sales))) if len(sales) > 2 else 0, last_actual * 0.015)
        allowed_rebound = daily_noise * np.sqrt(max(step, 1))
        cap = last_actual + allowed_rebound
        return max(0, min(prediction, cap))
    ...
    def _seasonal_trend_prediction(
        ...
        if len(recent) >= 14:
            ...
            if slope < 0 and y[-1] < float(np.median(y)):
                baseline = min(baseline, float(y[-1]))
                max_daily_move = max(float(np.std(y)) * 0.18, baseline * 0.08)
            else:
